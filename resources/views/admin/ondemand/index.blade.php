      ...
      <!--                           </td> -->
						...
						@foreach($getmon as $cat)
							<tr>
							   ...
							   @if($cat->assign == 1)
    							    @php $monthly = App\AdminModel\Pharmacy_ondemand::where('ondemand_id' , $cat->id)->first(); @endphp
    								<td>
								...
								@if(App\AdminModel\Pharmacy_ondemand::where('ondemand_id' , $cat->id)->exists())
    								@php
    								  $getphar = App\AdminModel\Pharmacy_ondemand::where('ondemand_id' , $cat->id)->first();
    								  $phar = App\AdminModel\Map::find($getphar->pharmacy_id);
    								@endphp
...
<!-- /.row -->
    ...
    function openNav(id) {
        ...
        $.get("{{ url('17$es12/ondemand/get_details_mon') }}?id="+id, function(data, status) {
            $( "#get_details" ).empty();
    ...
    function  ShowEdit(id) {
      $.get("{{ url('17$es12/ondemand/Getinfo') }}?id="+id, function(data, status){
          console.log(data)
          $("#id").val(id);
          $("#name").val(data.name);
    ...
    function feedback(id){
         $.get("{{ url('17$es12/ondemand/getfeedback') }}?id="+id, function(data, status){
             $('#get_feedback').empty();
     ...
     function undone(id){
         $.get("{{ url('17$es12/ondemand/getresoneundone') }}?id="+id, function(data, status){
             $('#get_undone').empty();
     ...
     function reasoncancelation(id){
         $.get("{{ url('17$es12/ondemand/getresone') }}?id="+id, function(data, status){
             $('#get_cancel').empty();
    ...
    function Delete() {
      ...
      $.get("{{ url('17$es12/ondemand/delete') }}?id="+id, function(data, status){
        location.reload();
          swal({
              position: 'top-center',
              type: 'success',
    ...
    function confirmed()
    {
          var call_id_get = $("#id_call").val();
           $.get("{{ url('17$es12/ondemand/confirm') }}?id="+call_id_get, function(data, status){
            console.log(data);
    ...
    function rejected()
    {
          var call_id_get = $("#id_call").val();
           ...
           $.get("{{ url('17$es12/ondemand/rejected') }}?id="+call_id_get, function(data, status){
        location.reload();
          swal({
              position: 'top-center',
              type: 'success',
...
<div class="add-popup modal fade" id="editproduct" tabindex="-1" role="dialog" aria-labelledby="editproduct">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<form action="{{url('17$es12/ondemand/update')}}" method="post" enctype="multipart/form-data">
				{{csrf_field()}}
...
<div class="add-popup modal fade" id="assign_modal" tabindex="-1" role="dialog" aria-labelledby="addproduct">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{url('17$es12/ondemand/assign')}}" method="post" enctype="multipart/form-data">
                {{csrf_field()}}
                    <div class="modal-body">
                    <input type="hidden" name="ondemand_id" id="id_assign">
