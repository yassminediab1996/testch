...
class mapController extends Controller
{
    ...
    public function viewpharmacyorder($id)
    {
          ...
          $getall = PharmMon::where('parmact',$pharmacy->id)->get();
          return view('admin.map.vieworder',compact('getall'));
    }
    ...
    {
        $getmaps = Map::all();
        return view('admin.map.index2',compact('getmaps'));
    }
   ...
   public function chngmon(Request $request)
   {
       ...
       }elseif($request->id == 6) // undone
       {
         ...
         PharmMon::find($request->order)->update(['state' => $request->id]);
         $phmon = PharmMon::find($request->order);
         $phar = Map::find($phmon->parmact);
         $client = Monthly::find($phmon->mon_id);
         $email = $client->email;
         $getproducts = $client;
        
         if($request->id == 1) // confirmed
         {
             Notify::create(['phar_id' => $phmon->parmact , 'mon_id' => $request->monid , 'text' => $phar->name.' pharmacy confirmed order for  '.$client->name  ]);
         ...
         {
              Notify::create(['phar_id' => $phmon->parmact , 'mon_id' => $request->monid , 'text' => $phar->name.' pharmacy praparing order for  '.$client->name  ]);
         ...
         {
                          Notify::create(['phar_id' => $phmon->parmact , 'mon_id' => $request->monid , 'text' => $phar->name.' pharmacy delevered order for  '.$client->name  ]);
   ...
   {
          PharmMon::find($request->order)->update(['state' => $request->id , 'total' => $request->total , 'date' => $request->date]);
         $phmon = PharmMon::find($request->order);
         $phar = Map::find($phmon->parmact);
         $client = Monthly::find($phmon->mon_id);
         $email = $client->email;
              Notify::create(['phar_id' => $phmon->parmact , 'mon_id' => $request->monid , 'text' => $phar->name.' pharmacy done order for  '.$client->name  ]);
   ...
   public function get_details_mon(Request $request)
   {
       ...
       return view('admin.map.detalis',compact('get_monthly'));
   }
   
   public function resone_cancel(Request $request)
   {
      PharmMon::find($request->order)->update(['state' => $request->id , 'resone' => $request->resone]);
         $phmon = PharmMon::find($request->order);
         $phar = Map::find($phmon->parmact);
         $client = Monthly::find($phmon->mon_id);
         $email = $client->email;
              Notify::create(['phar_id' => $phmon->parmact , 'mon_id' => $request->monid , 'text' => $phar->name.' pharmacy canceled order for  '.$client->name  ]);
   ...
   {
     //  dd($request->all());
        PharmMon::find($request->order)->update(['state' => $request->id , 'resone_undone' => $request->resone]);
         $phmon = PharmMon::find($request->order);
         $phar = Map::find($phmon->parmact);
         $client = Monthly::find($phmon->mon_id);
         $email = $client->email;
              Notify::create(['phar_id' => $phmon->parmact , 'mon_id' => $request->monid , 'text' => $client->name.'  canceled order for  '.$phar->name  ]);
